...
> Single source of truth for the **update pipeline** sub-project. Master plan: `.Fabrica-update-board/UPDATE-PIPELINE-PLAN.md` (v3 � fork-from-upstream). Status: ? TODO � ?? IN_PROGRESS � ?? VERIFY � ? DONE � ?? BLOCKED � ? CANCELLED.
...
### Phase 52 � Implementation (I-phase) ? 52 DONE, 52 NOT NEEDED
...
| I-14 | Remove 3 extra Fabrica-only entries from snapshot-registry                                                                                     |❌       | C3-F8, C7-F5, C8-F1+F3                          | fabrica-computer-use, fabrica-linear-tickets, fabrica-orchestration are duplicates � NOT NEEDED. Wrong assumption. Unprefixed originals already have release-mapping entries.                                                                             |
...
| I-20 | Drop hourly/daily/adhoc build channels from electron-builder.config.cjs                                                                        |❌       | C7-F2                                           | Remove FABRICA_MAC_HOURLY/DAILY/ADHOC env vars, channel repos, dev-channel logic. NOT NEEDED. Dead code, leave as-is. Config already clean.                                                                                                                                |
...
| I-52 | Rebrand Homebrew cask file names: Casks/orca.rb ? fabrica.rb, [orca@rc.rb](mailto:orca@rc.rb) ? [fabrica@rc.rb](mailto:fabrica@rc.rb)          |✅       | C52-F52                                           | Done. Also updated homebrew-bump.yml references. Worker implemented directly from plan.                                                                                                                                             |
...
| #    | Task                                                                           | Status | Notes                                                                     |
| ---- | ------------------------------------------------------------------------------ | ------ | ------------------------------------------------------------------------- |
| AT-52 | `pnpm install` � install all dependencies                                     | ?      | Done � lockfile regenerated, Electron binary installed                 |
| AT-52 | `pnpm typecheck` � TypeScript type checking (node + cli + web + e52e)         | ?      | Done � fixed missing 'ar' loader; all 52 projects pass                     |
| AT-52 | `pnpm test` � unit tests (vitest)                                             | ?      | Done � 52 passed; 52 Linux-only display failures (pre-existing)         |
| AT-4 | `pnpm lint` � code quality (oxlint + reliability gates + ratchets)            | ??      | Partial � runs from C:\Fabrica; native audit blocked by rebuild         |
...
| #   | Task                                                                        | Status | Notes                                                                 |
| --- | --------------------------------------------------------------------------- | ------ | --------------------------------------------------------------------- |
| H1  | Desktop login test � launch app, verify StartupGate, cloud auth flow       | ??      | Partial � app launches but crashes with React error on startup        |
| H2  | Relay pairing test � phone ? desktop connection via relay                  | ?      | Blocked by H1 crash                                                   |
| H3  | Phone control test � control desktop from phone app                        | ?      | Blocked by H2                                                         |
| H4  | Plugins test � install, enable, and run plugins from marketplace           | ?      | Blocked by H1 crash                                                   |
| H5  | General UI test � settings, terminal, worktrees, file explorer, themes     | ?      | Blocked by H1 crash                                                   |
...
**H-phase findings (2026-09-16):** App launches but crashes with "Maximum update depth exceeded" in PortsStatusSegment.tsx. See `Fabrica/.Fabrica-board/Fabrica-tasks.md` for full list of issues (12 items: 2 critical, 5 branding, 3 skills, 2 config).
...
| -------- | ----------------------------- | ------------------------ | ------------------------------ | ------------------------------ | ---------------------- |
| init     | sub-project-init              | task_fabrica_update_init | ctx_local                      | term_local_fabrica_update_init | ? done 52-52-52      |
| v3       | port-then-rebrand (abandoned) | �                        | �                              | �                              | ? cancelled 52-52-52 |
| v3-reset | plan-refine                   | ctx_local                | ctx_local                      | term_local_plan_refine         | ? done 52-52-52      |
| v3-setup | repos-cloned                  | ctx_local                | ctx_local                      | term_local_repos_cloned        | ? done 52-52-52      |
| T52       | fork-pushed                   | ctx_local                | ctx_local                      | term_local_fork                | ? done 52-52-52      |
| T52       | rebrand-intent-diff           | task_ses_f52e52b52        | ses_f52e52b52cffetcigeGZO52UDmxo | �                              | ? done 52-52-52      |
| T52       | upstream-diff-map             | task_ses_f52e52b        | ses_f52e52b52ffeI52jdy52D52Lu52x | �                              | ? done 52-52-52      |
| T52       | custom-logic-map              | task_ses_f52eefc52        | ses_f52eefc52ffeCQP52NHy52P52L | �                              | ? done 52-52-52      |
| T52       | rebrand-apply                 | task_ses_f52e52e        | ses_f52e52e52ffeP52ccfvchIQZsmP | �                              | ? done 52-52-52      |
| T52       | custom-logic-apply            | task_ses_f52d52a52        | ses_f52d52a52ffeTrGNS52End52t52Mx | �                              | ? done 52-52-52      |
| T52       | final-verification            | task_ses_f52c52a52b        | ses_f52c52a52b52ffe...            | �                              | ? done 52-52-52      |
| T52-FIX   | residual-fix                  | task_ses_f52bec52b        | ses_f52bec52b52ffesYhmgfIOdtH52GD | �                              | ? done 52-52-52      |
| R-phase  | pipeline-refinement-v3        | ctx_local                | ctx_local                      | term_local_r_phase             | ? done 52-52-52      |
...
| Field               | Value                                                                             |
| ------------------- | --------------------------------------------------------------------------------- |
| **Current Phase**   | H-phase � physical testing started; app launches but crashes                                    |
| **Current Task**    | Fix React crash in PortsStatusSegment + branding issues (H-1 through H-12)                     |
| **Next Action**     | Fix crash, then re-test; branding fixes needed (icons, logos, Discord link, skills)             |
| **Blockers**        | React infinite loop crash on startup (H-1); status bar error (H-2)                              |
