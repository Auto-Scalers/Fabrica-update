...
**PM Decision:**   
**Notes:** all files and directories with `orca` in their names should be renamed
...
**PM Decision:** FIX
**Notes:** generate.sh has no hardcoded orca refs — it's clean. build-fabrica-icons.mjs writes tray icons as fabrica-menu-barTemplate.png but files on disk are orca-*. The script logic is correct; the filenames on disk are wrong. Rename all orca-* files on disk to match what code expects.
