...
**PM Decision:**   
**Notes:** 30+ new upstream deps kept, should we rebrand any ?

### Finding 2 — electron-builder zero divergence (Info)

**PM Decision:** FIX
**Notes:** Drop hourly/daily/adhoc build channels entirely. Remove the dev-channel infrastructure from electron-builder.config.cjs. Keep only release (rc/patch/minor/major) builds. Simplifies config significantly.

### Finding 3 — locale-ko trim correct (Info)

**PM Decision:**   
**Notes:** how the old have 5,081 lines, the upstream have 15,069 lines but Fabrica/ have only 503 lines, how is this correct ?

### Finding 4 — Geist @font-face incomplete

**PM Decision:** 

**Notes: we shouold  comlete it**

### Finding 5 — snapshot-registry trim never applied

**PM Decision:** FIX
**Notes:** Remove the 3 extra Fabrica-only entries (fabrica-computer-use, fabrica-linear-tickets, fabrica-orchestration) from snapshot-registry. They duplicate the renamed versions. Rebrand ALL 9 skills: orca-* → fabrica-*, unprefixed get fabrica- prefix. Apply to registries AND directories.

### Finding 6 — release-mapping trim never applied

**PM Decision:** FIX
**Notes:** Keep full upstream (no trim needed). But rebrand ALL skill names: orca-* → fabrica-*, unprefixed get fabrica- prefix. Apply to release-mapping.json.

### Finding 7 — StartupGate orphaned dead code

**PM Decision:** FIX
**Notes:** Wire StartupGate into the app render tree. Add local fallback (offline mode). Fix casing: FABRICA → fabrica. Connect to actual auth flow (custom OAuth 2.0/PKCE).
