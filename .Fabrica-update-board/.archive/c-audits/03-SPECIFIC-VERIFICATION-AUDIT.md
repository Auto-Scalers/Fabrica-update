...
**PM Decision:** FIX
**Notes:** Replace @fabrica-ai/playwright-test with @playwright/test (original). No functionality loss — @fabrica-ai/playwright-test is a thin wrapper that re-exports standard Playwright APIs with Electron config.

### Finding 2 — Relay server split complete (Pass)

**PM Decision:** FIX
**Notes:** Fully drop Fabrica-relay/ (Cloudflare Workers). Adopt upstream cloud/apps/relay/ (GCE production infrastructure). Last task — needs planning with PM.

### Finding 3 — PascalCase cleaned (Pass)

**PM Decision:** FIX
**Notes:** Rebrand gitignore patterns (.stably/* → .fabrica/*) and Linear slug. The Linear slug is already `fabrica` (linear.app/fabrica/).

### Finding 4 — Filenames intentionally preserved (Pass)

**PM Decision:** FIX
**Notes:** Not intentionally preserved — incomplete rebrand. Rename ALL orca-named files and directories. Rename orca-ide → fabrica (no -ide). Remove GNOME screen reader mentions. Update all references (~399 across ~160 files).
...
**PM Decision:** FIX
**Notes:** Rebrand all ORCA_* env vars to FABRICA_*, fix SignPath project-slug, rebrand Homebrew cask file names. Task before last — needs planning with PM.

### Finding 7 — No BOM in package.json (Pass)

**PM Decision:** SKIP
**Notes:** No BOM in package.json. Locale file BOM stripping is separate (covered in C10 audit).

### Finding 8 — 3 new Fabrica skills (Pass)

**PM Decision:** FIX
**Notes:** Remove the 3 extra Fabrica-only entries (fabrica-computer-use, fabrica-linear-tickets, fabrica-orchestration) from snapshot-registry — they duplicate the renamed versions. The unprefixed originals (computer-use, linear-tickets, orchestration) already have release-mapping entries. Rebrand ALL 9 skills: orca-* → fabrica-*, unprefixed get fabrica- prefix. Apply to both directories on disk AND registries (current-manifest.json, snapshot-registry.json, release-mapping.json).

### Finding 9 — eas.json uses profiles not channels (Pass)

**PM Decision:** SKIP
**Notes:** Build profiles match. No changes needed. Hourly/daily/adhoc channels are being dropped.