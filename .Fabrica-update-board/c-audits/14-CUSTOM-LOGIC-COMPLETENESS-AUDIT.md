...
**PM Decision:** FIX
**Notes:** Use `ai.autoscalers.fabrica` as canonical app ID everywhere. Align electron-builder config and compatibility contract. Verify test electron-builder-config.test.mjs.

For relay fallback: it's upstream code (not custom). The `src/relay/` uses Node.js + WebSocket + Hono. Cloud infrastructure uses GCE + Cloud SQL + Terraform. Since you don't have a GCE account and were using Cloudflare, you'd need to set up server-side infra or adapt the relay server code.

For skills: remove the 3 extra Fabrica-only entries from snapshot-registry. Rebrand ALL 9 skills with fabrica- prefix (orca-* → fabrica-*, unprefixed → fabrica-*). Apply to both directories on disk AND registries.

### Finding 2 — Mobile bundle ID mismatch (MEDIUM)

**PM Decision:** FIX
**Notes:** Use `ai.autoscalers.fabrica` as canonical everywhere. Desktop = `ai.autoscalers.fabrica`. Mobile = `ai.autoscalers.fabrica` (or `ai.autoscalers.fabrica.mobile` if separate mobile ID needed). Update SIGNING.md. No more com.fabrica.fabrica.mobile or com.fabrica-ai.fabrica.

### Finding 3 — Font migration incomplete (MEDIUM)

**PM Decision:** FIX
**Notes:** Complete it — add @font-face for Geist to main.css and update :root --app-font-family.

### Finding 4 — docs/ai-vault-process-isolation-plan.md missing (LOW)

**PM Decision:** SKIP
**Notes:** Low priority. Can be recovered from old repo if needed later.

### Finding 5 — No NOTICE or CREDITS files (INFO)

**PM Decision:** SKIP
**Notes:** Not required for this rebrand.
